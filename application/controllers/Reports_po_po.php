...
class Reports_po_po extends Root_Controller
{
    private $message;
    public $permissions;
    ...
    public function __construct()
    {
        parent::__construct();
        $this->message="";
        $this->permissions=User_helper::get_permission('Reports_po_po');
        ...
        $this->controller_url='reports_po_po';
    }
    ...
    public function get_items()
    {
        ...
        $status_requested=$this->input->post('status_requested');
        $status_approved=$this->input->post('status_approved');
        $status_delivered=$this->input->post('status_delivered');
        ...
        $this->db->select('po.id,po.customer_id outlet_id,po.date_po,po.date_created');
        ...
        $this->db->where('po.status_requested',$status_requested);
        if($status_approved!='')
        {
            $this->db->where('po.status_approved',$status_approved);
        }
        if($status_delivered!='')
        {
            $this->db->where('po.status_delivered',$status_delivered);
        }
        if($status_received!='')
        {
            $this->db->where('po.status_received',$status_received);
        }

        if($date_end>0)
        {
            $this->db->where('po.date_po <=',$date_end);
        }
        if($date_start>0)
        {
            $this->db->where('po.date_po >=',$date_start);
        }
        ...
        foreach($items as &$item)
        {
            ...
            $item['time_po']=System_helper::display_date_time($item['date_created']);
        }
