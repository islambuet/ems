...
?>
    ...
    if($setup['zone_id']>0)
    {
        $CI->load->view("setup_tm_zi_market_visit/schedule",array('zone_id'=>$setup['zone_id']));
    }
    ?>
    </div>
...
jQuery(document).ready(function()
{
    ...
    $(document).on("change",".territory_id",function()
    {
        ...
        $('#district_container_'+day_no+'_'+shift_id).html('');
        $('#customers_container_'+day_no+'_'+shift_id).html('');

        if(territory_id>0)
        {
            $.ajax({
                url: "<?php echo site_url($CI->controller_url.'/get_districts')?>",
                type: 'POST',
                datatype: "JSON",
                data:{territory_id:territory_id,day_no:day_no,shift_id:shift_id},
                success: function (data, status)
                {
