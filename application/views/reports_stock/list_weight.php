...
<script type="text/javascript">
    $(document).ready(function ()
    {
           ...
           // console.log(defaultHtml);
            ...
            if (record.crop_type_name=="Total Type")
            {
                ...
                if(column=='starting_stock')
                {
                    ...
                    element.html(number_format(type_starting_stock,3,'.',''));
                    type_starting_stock=0;
                }
                else if(column=='stock_in')
                {
                    element.html(number_format(type_stock_in,3,'.',''));
                    type_stock_in=0;
                }
            }
            else if (record.crop_name=="Total Crop")
            {
                ...
                if(column=='starting_stock')
                {
                    ...
                    element.html(number_format(crop_starting_stock,3,'.',''));
                    crop_starting_stock=0;
                }
                else if(column=='stock_in')
                {
                    //element.html(crop_stock_in);
                    element.html(number_format(crop_stock_in,3,'.',''));
                    crop_stock_in=0;
                }
            }
            else
            {
                if(column=='starting_stock')
                {
                    ...
                    var weight=number_format((record.pack_size_name*value/1000),3,'.','');
                    element.html(weight);
        ...
        var grand_starting_stock=function (total, column, element, record)
        {
            if(element=="starting_stock")
            {
                ...
                if(qtn)
                {
                    ...
                    return total+qtn*record.pack_size_name;
                }
        ...
        var aggregatesrenderer=function (aggregates)
            {
                return '<div style="position: relative; margin: 0px;padding: 4px;width: 100%;height: 100%; overflow: hidden;">' +number_format(aggregates['total']/1000,3,'.','')+'</div>';
